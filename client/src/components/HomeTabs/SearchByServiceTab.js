...
// Search by service tab component definition
const SearchByServiceTab = () => {
  ...
  // useEffect hook to load the list of services and fetch data
  useEffect(() => {
    // Load the services list
    ServiceDataService.getAll().then((response) => {
  ...
  const handleServiceSelect = async (event) => {
    ...
    // To load the employees by the selectioned service
    if (serviceId) {
      try {
        const response = await EmployeeDataService.getByService(serviceId);
  ...
  return (
    <div className="row d-flex justify-content-center">
      ...
      <div className="col-md-10">
        <Form.Select id="searchByService" onChange={handleServiceSelect}>
          <option value="" className="text-center">
            Sélectionnez un service
          </option>
          {services.map((service) => (
            <option key={service.id} className="text-center" value={service.id}>
              {service.name}
            </option>
          ))}
        </Form.Select>
      ...
      <div className="col-md-6 mt-3">
        {currentEmployee ? (
          ...
          <div>{selectedService && <p>Veuillez cliquer sur un employé</p>}</div>
        )}
      </div>
    </div>
  );
...
export default SearchByServiceTab;